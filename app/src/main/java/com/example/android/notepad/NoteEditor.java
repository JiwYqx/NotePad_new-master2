    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.menu_revert:
                ...
                break;
            // 添加新的排序菜单项处理
            case R.id.menu_sort: // 假设您在 XML 中定义了这个菜单项
                toggleSortOrder(); // 调用排序方法
...
//END_INCLUDE(paste)
    ...
    private void toggleSortOrder() {
        isSortedAscending = !isSortedAscending; // 切换排序状态

        // 根据排序状态构建排序字符串
        String sortOrder = isSortedAscending ? NotePad.Notes.COLUMN_NAME_MODIFICATION_DATE + " ASC" : NotePad.Notes.COLUMN_NAME_MODIFICATION_DATE + " DESC";

        // 重新查询便签，按时间排序
        mCursor = getContentResolver().query(mUri, PROJECTION, null, null, sortOrder);
        
        // 更新UI或其他操作
        // 例如，您可能需要更新ListView的适配器
        // adapter.notifyDataSetChanged(); // 如果您使用适配器
    }
}
